...
# ─── SITEMAP ─────────────────────────────────────────────
# frontend/vercel.json rewrites https://pashx.com/sitemap.xml here, so this
# is the one canonical sitemap (matches robots.txt's Sitemap: directive).
# Generated live from the DB — every published post is included the moment
# it's published, nothing to resubmit or regenerate by hand.
...
_STATIC_PAGES = [
    ...
    ("/contact", "yearly", "0.7"),
    ("/book-demo", "yearly", "0.7"),
]


@app.get("/sitemap.xml")
async def sitemap():
    """Static marketing pages + every published blog post, generated live."""
    from fastapi.responses import Response

    urls = []
    for path, changefreq, priority in _STATIC_PAGES:
        urls.append(
            f"<url><loc>https://pashx.com{path}</loc>"
            f"<changefreq>{changefreq}</changefreq><priority>{priority}</priority></url>"
        )

    blogs = await db_instance.blogs.find(
    ...
    for b in blogs:
        ...
        urls.append(
            f"<url><loc>https://pashx.com/blog/{slug}</loc>{lastmod}"
